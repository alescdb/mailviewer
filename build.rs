...
fn glib_compile_resources(cfg: &Config) {
  ...
  let output: std::process::Output = Command::new("glib-compile-resources")
    .arg("--sourcedir=src")
  ...
  println!("cargo:warning=glib_compile_resources => {:?}", &output);
}
...
fn glib_compile_schemas(cfg: &Config) {
  ...
  println!("cargo:warning=glib_compile_schemas => {:?}", &output);
}
